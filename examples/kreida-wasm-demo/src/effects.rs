...
pub fn wave(buffer: &mut Buffer, phi: f64) {
...
pub fn lines(buffer: &mut Buffer, phi: f64) {
...
pub fn spirograph(buffer: &mut Buffer, phi: f64) {
    let w_2: isize = buffer.width() as isize / 2;
    let h_2: isize = buffer.height() as isize / 2;
    let r = min(h_2, w_2) as f64;
...
pub fn sinusoid1(buffer: &mut Buffer, phi: f64) {
...
pub fn sinusoid2(buffer: &mut Buffer, phi: f64) {
    let h_2 = (buffer.height() / 2) as isize;
